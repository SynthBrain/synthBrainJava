...
public class ControllerVision extends VisionCortex {
    ...
    /**
     * Частота служит для обьединения в группы
     * несколько групп с одной частотой ,один мысле-образ
     * нейрон представитель -> нейрон детектор -> группа однородных детекторов в границах фокуса
     * -> их представитель -> группа представителей -> представитель общей частоты
     *
     *
     * цвет имеет своего представителя
     * интенсивность или цвет входящих сигналов роли не играет, лиш однородность того что в фокусе,
     * скажем так - запоминаються пятна одной интенсивности, после представители этих пятен работают на общей частоте
     * и запоминаеться эта группа формируя представителя, что равно всему образу что пришел на вход
     */
    private void logic(){
     ...
     * Создаём связи
     *
     *  + пришел стимул нужно найти в облаке аналогичный занести его вектор в графу активности
     *  + добавить в список стимулов с которыми есть связь(этот этап загружен при инициализации)
     * добавить в список елементов с одинаковой активностью
     * создать представитель паттерна, для этой группы
     * создать такие же группы для оставшихся видов стимулов
     * создать представителя уже для етих групп
     *
     */

    /**
     * Группируем одинаковое и создаём паттерн который сравниваем с якорем уровня (паттерн от которого выставляем в многомерном пространстве остальные)
     * после этого сравниваем его с паттернами что уже находяться в радиусе тех координат куда он попал
     * есть 100% совпадение то усиливаем тот паттерн, нет создаём ему тело в тех координатах
     */
