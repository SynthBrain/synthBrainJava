...
public class ControllerVision extends VisionCortex {
     ...
     * как и очень точные. но находиться в пространстве должны рядом (похожее рядом), так же по мере уточнения образов создавать связи между абстрактными и более точными
     *
     * Фокус зрения наводиться на самые сильные разницы общей панорамы зрения -- приоритет на движение (лучше удволетворяет потребность новизны)
     * иначе быстро становиться скучно и продолжает дальше изучать панораму
     *
     */
    ...
    private Vector3 createTotalAverage(ArrayList<Vector3> list){
        ...
        average.x = (float) Math.round(average.x * 10f) / 10f;
        average.z = (float) Math.round(average.z * 10f) / 10f;
        //System.out.println(average);
